...
class AnthropicLargeLanguageModel(LargeLanguageModel):
    ...
    def _to_credential_kwargs(self, credentials: dict) -> dict:
        ...
        """
        credentials_kwargs = {
            "api_key": credentials["sibilare_anthropic_api_key"],
            "timeout": Timeout(315.0, read=300.0, write=10.0, connect=5.0),
            "max_retries": 1,
        }

        if credentials.get("sibilare_anthropic_api_url"):
            credentials["sibilare_anthropic_api_url"] = credentials["anthropic_api_url"].rstrip("/")
            credentials_kwargs["base_url"] = credentials["sibilare_anthropic_api_url"]
